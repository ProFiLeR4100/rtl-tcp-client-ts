...
describe(`integration: real rtl_tcp server @ ${HOST}:${PORT}`, () => {
  it('handshakes, applies settings and streams live IQ samples', { timeout: 30_000 }, async () => {
    const client = new RtlSdrClient({ host: HOST, port: PORT, connectTimeoutMs: 10_000, chunkSize: 1024 });
    try {
      ...
      console.log(`tuner: ${client.tunerName} (type=${client.tunerType}, gain steps=${client.tunerGainCount})`);
