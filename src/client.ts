...
import { decodeIq, encodeCommand, parseHandshake, Handshake } from './protocol';
...
export class RtlSdrClient extends EventEmitter {
  ...
  get connected(): boolean { return this._state === 'stream'; }
  get tunerType(): number | undefined { return this._handshake?.tunerType; }
  get tunerGainCount(): number | undefined { return this._handshake?.tunerGainCount; }
  ...
  get bufferedSamples(): number { return (this._rx.length >> 2) | 0; }
  get totalSamplesEmitted(): number { return this._totalIq; }
  get droppedSamples(): number { return this._totalDropped; }
  ...
  connect(): Promise<RtlSdrClient> {
    ...
    if (this._socket) return Promise.reject(new Error('A connection is already in progress or established.'));

    const socket = net.connect({ host: this._host, port: this._port, timeout: this._connectTimeoutMs });
    this._socket = socket;
  ...
  setAutoGain(auto: boolean): Promise<void> { return this.setGainMode(auto); }
  ...
  setTestMode(on: boolean): Promise<void> { return this._sendCommand(COMMANDS.TEST_MODE, on ? 1 : 0); }
  setAfcMode(on: boolean): Promise<void> { return this._sendCommand(COMMANDS.AFC_MODE, on ? 1 : 0); }
  setDirectSampling(mode: 0 | 1 | 2): Promise<void> {
    if (mode !== 0 && mode !== 1 && mode !== 2) throw new RangeError('direct sampling mode must be 0, 1 or 2');
    return this._sendCommand(COMMANDS.DIRECT_SAMPLING, mode);
  }
  setOffsetTuning(on: boolean): Promise<void> { return this._sendCommand(COMMANDS.OFFSET_TUNING, on ? 1 : 0); }
  setRtlXtal(hz: number): Promise<void> { this._checkInt(hz, 'RTL xtal'); return this._sendCommand(COMMANDS.RTL_XTAL, hz); }
  setTunerXtal(hz: number): Promise<void> { this._checkInt(hz, 'tuner xtal'); return this._sendCommand(COMMANDS.TUNER_XTAL, hz); }
  setGainByIndex(index: number): Promise<void> { this._checkInt(index, 'gain index'); return this._sendCommand(COMMANDS.GAIN_BY_INDEX, index); }
  setBiasTee(on: boolean): Promise<void> { return this._sendCommand(COMMANDS.BIAS_TEE, on ? 1 : 0); }
  ...
  // ---- data ---------------------------------------------------------------
  onSamples(cb: (iq: Int16Array) => void): this { return this.on('samples', cb); }
  pause(): this { this._socket?.pause(); return this; }
  resume(): this { this._socket?.resume(); return this; }
